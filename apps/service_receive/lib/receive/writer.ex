defmodule Receive.Writer do
  ...
  getter(:dlq, default: Receive.DLQ)
    ...
    results =
      Enum.reduce(messages, %{ok: [], error: []}, fn message, acc ->
        case Jason.encode(message) do
          {:ok, json} -> Map.update!(acc, :ok, fn list -> [json | list] end)
          {:error, reason} -> Map.update!(acc, :error, fn list -> [{message, reason} | list] end)
        end
      end)
  ...
  defp to_dead_letter(dataset_id, {raw, reason}) do
    DeadLetter.new(
