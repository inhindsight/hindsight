defmodule Define.Store do
  require Logger
  alias Define.{StepView, StepFieldView, DictionaryView, DictionaryFieldView, TypespecAnalysis, DictionarySerialization}
  ...
  def update_definition(%Extract{} = data) do
    ...
    |> Map.put(:dictionary, DictionarySerialization.serialize(data.dictionary))
    |> persist()
  end
  ...
  defp serialize_step(step) do
    ...
    new_fields =
      Enum.map(types, fn {k, v} -> %StepFieldView{key: k, type: v, value: Map.get(step, k)} end)
