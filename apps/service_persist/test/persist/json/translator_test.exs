defmodule Persist.Json.TranslatorTest do
  use ExUnit.Case
  import Checkov

  data_test "translate #{input} to json" do
    assert {:ok, output} == Persist.Json.Translator.translate(type, input)
  ...
  test "translates values in simple list" do
    ...
    assert {:ok, ["2020-01-01 01:27:41.0"]} ==
             Persist.Json.Translator.translate(list, ["2020-01-01T01:27:41Z"])
  end
  ...
  test "translates values in a map" do
    ...
    assert {:ok, expected} == Persist.Json.Translator.translate(map, input)
  end
end
