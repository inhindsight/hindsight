...
defmodule BroadwayConfigurator.Default do
  ...
  @callback configure_producer(config :: keyword, context :: term) ::
              {:ok, keyword} | {:error, term}
  @callback configure_processors(config :: keyword, context :: term) ::
              {:ok, keyword} | {:error, term}
  @callback configure_batchers(config :: keyword, context :: term) ::
              {:ok, keyword} | {:error, term}
    ...
    quote do
      ...
      def configure(config, context) do
        with {:ok, name} <- name(context),
             ...
             {:ok, processors} <-
               configure_processors(Keyword.get(config, :processors, []), context),
